...
nParticles = 54;
nBins = 5;
for isWeber = -1:1:1; % if use Weber Law type response instead of diffusion
    ...
    xTickLabels = {'x - 2\Delta x', 'x - \Delta x', 'x', 'x + \Delta x', 'x + 2\Delta x'};
    ...
    xBinRand = [ones(6,1); 2*ones(6,1); 3*ones(18,1); 4*ones(18,1); 5*ones(6,1);];
    ...
    for kk = 1:nParticles*12005
        ...
        if mod(kk, nParticles*6000) == 0 % || mod(kk, nParticles*6000) == nParticles*1000
            ...
            for jj = 1: (length(nInEachBin) - 1)
                text(xCenterBins(jj) - 0.2, 0.7, sprintf('N = %d', nInEachBin(jj)), 'FontName', 'arial', 'FontSize', useFontSize);
            end
            ...
            if isWeber == 0
                % directly move the particles
                particlesToMoveRight = [1, 7, 13, 14, 15, 31, 32, 33, 49];
                particlesToMoveLeft = [2, 8, 16, 17, 18, 34, 35, 36, 50];
                xPos(particlesToMoveRight) = mod(xPos(particlesToMoveRight)+1, nBins);
                xPos(particlesToMoveLeft) = mod(nBins + xPos(particlesToMoveLeft)-1, nBins);
                %randperm(nParticles);
                %oneSixth = round(nParticles/6); twoOverSix = round(nParticles/3); % move one third of the particles
                %xPos(particlesToMove(1:oneSixth)) = min(xPos(particlesToMove(1:oneSixth)), mod(nBins + xPos(particlesToMove(1:oneSixth)) -1, nBins));
                %xPos(particlesToMove(oneSixth+1:twoOverSix)) =  max( xPos(particlesToMove(oneSixth+1:twoOverSix)), mod(xPos(particlesToMove(oneSixth+1:twoOverSix)) + 1, nBins));
                
            elseif isWeber > 0 % climb the gradient
                % directly move the particles
                particlesToMoveRight = [7, 8];
                particlesToMoveLeft = [50, 51];
                xPos(particlesToMoveRight) = mod(xPos(particlesToMoveRight)+1, nBins);
                xPos(particlesToMoveLeft) = mod(nBins + xPos(particlesToMoveLeft)-1, nBins);
            elseif (isWeber < 0) % down the gradient
                particlesToMoveLeft = [13, 14];
                particlesToMoveRight = [34, 35];
                xPos(particlesToMoveRight) = mod(xPos(particlesToMoveRight)+1, nBins);
                xPos(particlesToMoveLeft) = mod(nBins + xPos(particlesToMoveLeft)-1, nBins);
